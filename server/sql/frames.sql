...
CREATE OR REPLACE TABLE
    frame_details_reference_ids (
        ...
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        status INT NOT NULL DEFAULT 1,
        CONSTRAINT unique_frame UNIQUE (
            f_company_id,
            f_material_id,
            f_model_id,
            f_size_id
...
CREATE OR REPLACE TABLE
    frame_details (
        ...
        f_qty INT NOT NULL,
        f_price_id INT REFERENCES frame_prices (id) ON UPDATE CASCADE,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        status INT NOT NULL DEFAULT 1,
        CONSTRAINT unique_frame_code UNIQUE (f_code)
    );